...
function auth(req: NextRequest): any | null {
...
export async function GET(req: NextRequest, { params }: { params: { path: string[] } }) {
  return handleRoute(req, params.path, "GET");
}

export async function POST(req: NextRequest, { params }: { params: { path: string[] } }) {
  return handleRoute(req, params.path, "POST");
}

export async function PUT(req: NextRequest, { params }: { params: { path: string[] } }) {
  return handleRoute(req, params.path, "PUT");
}

export async function DELETE(req: NextRequest, { params }: { params: { path: string[] } }) {
  return handleRoute(req, params.path, "DELETE");
}
...
async function handleRoute(req: NextRequest, path: string[], method: string): Promise<NextResponse> {
  try {
    ...
    if (collection === "health") return NextResponse.json({ status: "ok" });

    if (collection === "auth") return await handleAuth(req, id, method);
    if (collection === "analytics") return await handleAnalytics(req, id, method);

    const user = auth(req);
    if (!user && collection !== "health") return NextResponse.json({ error: "Unauthorized" }, { status: 401 });

    const scope = deptScope(user);

    switch (collection) {
      case "departments": return await handleCrud("departments", req, id, method, scope, ["name", "code"], true);
      case "faculties": return await handleCrud("faculties", req, id, method, scope, ["name", "email", "departmentId"], true);
      case "students": return await handleCrud("students", req, id, method, scope, ["name", "rollNumber", "departmentId"], true);
      case "publications": return await handleCrud("publications", req, id, method, scope, ["title", "facultyId"], true);
      case "patents": return await handleCrud("patents", req, id, method, scope, ["title", "facultyId"], true);
      case "research": return await handleCrud("research", req, id, method, scope, ["title", "facultyId"], true);
      case "events": return await handleCrud("events", req, id, method, scope, ["title", "startDate", "departmentId"], true);
      case "targets": return await handleCrud("targets", req, id, method, scope, ["metric", "targetValue", "academicYear", "departmentId"], true);
      case "notifications": return await handleCrud("notifications", req, id, method, scope, ["userId", "title", "message"], false);
      case "documents": return await handleCrud("documents", req, id, method, scope, ["title", "fileName"], true);
      case "signatures": return await handleCrud("signatures", req, id, method, scope, ["userId", "reportId", "signatureData"], false);
...
async function handleAnalytics(_req: NextRequest, action: string | undefined, _method: string) {
  const d = getDb();
  if (action === "dashboard") {
...
async function handleCrud(col: string, req: NextRequest, id: string | undefined, method: string, scope: string | null, required: string[], enrich: boolean) {
  const d = getDb();
  if (method === "GET" && !id) {
    ...
    if (scope) {
      ...
      } else if (col !== "signatures" && col !== "notifications") {
...
async function handleReports(req: NextRequest, id: string | undefined, action: string | undefined, method: string, user: any, scope: string | null) {
  ...
  if (method === "POST" && action === "generate") {
    const body = await req.json();
    if (!body.title || !body.type || !body.departmentId) return NextResponse.json({ error: "title, type, departmentId required" }, { status: 400 });
    const nid = uuid();
  ...
  if (method === "PUT" && id && !action) {
    const doc = await d.collection("reports").doc(id).get();
    if (!doc.exists) return NextResponse.json({ error: "Not found" }, { status: 404 });
    if (doc.data()?.status === "LOCKED") return NextResponse.json({ error: "Cannot edit locked report" }, { status: 400 });
    const body = await req.json();
    const ud = { ...body, updatedAt: new Date().toISOString() };
  ...
  if (method === "DELETE" && id) {
    const doc = await d.collection("reports").doc(id).get();
    if (!doc.exists) return NextResponse.json({ error: "Not found" }, { status: 404 });
    if (doc.data()?.status === "LOCKED") return NextResponse.json({ error: "Cannot delete locked" }, { status: 400 });
  ...
  if (method === "POST" && action === "submit" && id) {
    const doc = await d.collection("reports").doc(id).get();
    if (!doc.exists) return NextResponse.json({ error: "Not found" }, { status: 404 });
    if (doc.data()?.status !== "DRAFT") return NextResponse.json({ error: "Only DRAFT can be submitted" }, { status: 400 });
    await d.collection("reports").doc(id).update({ status: "SUBMITTED", submittedAt: new Date().toISOString(), updatedAt: new Date().toISOString() });
...
async function handleApprovals(req: NextRequest, id: string | undefined, action: string | undefined, method: string, user: any) {
  ...
  if (method === "POST" && !id) {
    ...
    if (!reportId || !level) return NextResponse.json({ error: "reportId and level required" }, { status: 400 });
    const rd = await d.collection("reports").doc(reportId).get();
    if (!rd.exists) return NextResponse.json({ error: "Report not found" }, { status: 404 });
    const nid = uuid();
    ...
    await d.collection("approvals").doc(nid).set(data);
    return NextResponse.json({ id: nid, ...data }, { status: 201 });
  }
  if (method === "PUT" && id && action === "approve") {
    ...
    if (li + 1 < APPROVAL_LEVELS.length) {
      ...
      else await d.collection("approvals").doc(uuid()).set({ reportId: a.reportId, userId: user.id, level: next, status: "PENDING", comment: `Auto after ${a.level}`, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() });
    }
...
async function handleUsers(req: NextRequest, id: string | undefined, method: string, scope: string | null) {
  ...
  if (method === "POST" && !id) {
    ...
    const ex = await d.collection("users").where("email", "==", email).limit(1).get();
    if (!ex.empty) return NextResponse.json({ error: "Email exists" }, { status: 400 });
    ...
    return NextResponse.json({ id: nid, email, name, role, departmentId }, { status: 201 });
  }
  return NextResponse.json({ error: "Not found" }, { status: 404 });
}
