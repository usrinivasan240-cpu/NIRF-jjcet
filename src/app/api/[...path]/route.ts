...
async function handleRoute(req: NextRequest, path: string[], method: string): Promise<NextResponse> {
  try {
    ...
    switch (col) {
      ...
      case "faculties":
      case "faculty": return await handleCrud("faculties", req, id, method, scope);
      ...
      case "notifications": return await handleNotifications(req, id, action, method);
...
async function handleAuth(req: NextRequest, action: string | undefined, method: string) {
  if (action === "login" && method === "POST") {
    ...
    if (mu) {
      ...
      return ok({ token, user: { id: mu.id, email: mu.email, name: mu.name, role: mu.role, departmentId: mu.departmentId } });
    }
    ...
    try {
      ...
      return ok({ token, user: { id: snap.docs[0].id, email: u.email, name: u.name, role: u.role, departmentId: u.departmentId } });
    } catch (e: any) {
      return err("Invalid credentials: " + e.message, 401);
    }
  }
  if (action === "profile" && method === "GET") {
    ...
    if (mu) return ok({ id: mu.id, email: mu.email, name: mu.name, role: mu.role, departmentId: mu.departmentId });
    try {
      const d = await getDb();
      const doc = await d.collection("users").doc(user.id).get();
      if (!doc.exists) return err("Not found", 404);
      const u = doc.data()!;
      return ok({ id: doc.id, email: u.email, name: u.name, role: u.role, departmentId: u.departmentId });
    } catch { return err("Not found", 404); }
  }
...
async function handleAnalytics(action: string | undefined, _method: string) {
  try {
    ...
    if (!action || action === "dashboard") {
      const [fac, stu, dep, pub, pat, evt, res] = await Promise.all([
        ...
        d.collection("research").get(),
      ]);
      const reportsSnap = await d.collection("reports").get();
      return ok({
        departments: dep.size, faculty: fac.size, publications: pub.size,
        patents: pat.size, research: res.size, students: stu.size,
        events: evt.size, reports: reportsSnap.size,
        targetAchievement: 0, conferences: 0,
      });
    }
...
async function handleSettings(req: NextRequest, method: string) {
  try {
    const d = await getDb();
    if (method === "GET") {
      try {
        const doc = await d.collection("settings").doc("global").get();
        if (doc.exists) return ok({ id: doc.id, ...doc.data() });
      } catch {}
      return ok({ collegeName: "J.J. College of Engineering & Technology", collegeCode: "JJCET" });
    }
    if (method === "PUT") {
      const body = await req.json();
      await d.collection("settings").doc("global").set({ ...body, updatedAt: new Date().toISOString() }, { merge: true });
      return ok(body);
    }
  } catch (e: any) {
    return err(e.message, 500);
...
async function handleCrud(col: string, req: NextRequest, id: string | undefined, method: string, scope: string | null) {
  ...
  if (method === "GET" && !id) {
    ...
    if (scope) {
      ...
      } else if (!["signatures"].includes(col)) {
        q = q.where("departmentId", "==", scope);
      }
    }
    const snap = await q.get();
    let items = snap.docs.map((dd: any) => ({ id: dd.id, ...dd.data() }));
    if (!["signatures"].includes(col)) items = await Promise.all(items.map(i => enrichItem(col, i)));
    return ok(items);
  }
  if (method === "GET" && id) {
    ...
    if (!["signatures"].includes(col)) item = await enrichItem(col, item);
    return ok(item);
  }
  if (method === "POST") {
    const body = await req.json();
...
async function handleNotifications(req: NextRequest, id: string | undefined, action: string | undefined, method: string) {
  const d = await getDb();
  if (method === "GET" && !id) {
    const snap = await d.collection("notifications").orderBy("createdAt", "desc").get();
    const items = snap.docs.map((dd: any) => ({ id: dd.id, ...dd.data() }));
    return ok(items);
  }
  if (method === "PUT" && id && action === "read") {
    await d.collection("notifications").doc(id).update({ read: true });
    return ok({ message: "Marked as read" });
  }
  if (method === "PUT" && id === "read-all") {
    const snap = await d.collection("notifications").where("read", "!=", true).get();
    const batch = d.batch();
...
async function enrichItem(col: string, item: any) {
  ...
  if (r.userId && col === "approvals") {
    ...
    if (ud.exists) { const u = ud.data()!; r.user = { id: ud.id, name: u.name, email: u.email, role: u.role }; }
  }
  if (r.reportId && col === "approvals") {
    try {
      const rd = await d.collection("reports").doc(r.reportId).get();
      if (rd.exists) r.report = { id: rd.id, ...rd.data() };
    } catch {}
  }
  return r;
}
...
async function handleReports(req: NextRequest, id: string | undefined, action: string | undefined, method: string, user: any, scope: string | null) {
  const d = await getDb();
  const reportActions = ["templates", "generate"];
  const effectiveAction = action || (id && reportActions.includes(id) ? id : undefined);
  const effectiveId = id && !reportActions.includes(id) ? id : undefined;

  if (method === "GET" && effectiveAction === "templates") return ok(REPORT_TEMPLATES);
  if (method === "GET" && !effectiveId) {
  ...
  if (method === "GET" && effectiveId) {
    const doc = await d.collection("reports").doc(effectiveId).get();
    if (!doc.exists) return err("Not found", 404);
    const r = await enrichItem("reports", { id: doc.id, ...doc.data() });
    const [as, ss] = await Promise.all([d.collection("approvals").where("reportId", "==", effectiveId).get(), d.collection("signatures").where("reportId", "==", effectiveId).get()]);
    r.approvals = as.docs.map((dd: any) => ({ id: dd.id, ...dd.data() }));
    r.signatures = ss.docs.map((dd: any) => ({ id: dd.id, ...dd.data() }));
    return ok(r);
  }
  if (method === "POST" && effectiveAction === "generate") {
    const body = await req.json();
    const nid = await genId();
    const data = { ...body, status: "DRAFT", currentLevel: "STAFF", creatorId: user.id, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() };
    await d.collection("reports").doc(nid).set(data);
    return ok({ id: nid, ...data }, 201);
  }
  if (method === "PUT" && effectiveId) {
    const doc = await d.collection("reports").doc(effectiveId).get();
    if (!doc.exists) return err("Not found", 404);
    if (doc.data()?.status === "LOCKED") return err("Locked", 400);
    const body = await req.json();
    const ud = { ...body, updatedAt: new Date().toISOString() };
    delete ud.id; delete ud.createdAt; delete ud.status;
    await d.collection("reports").doc(effectiveId).update(ud);
    return ok({ id: effectiveId, ...doc.data(), ...ud });
  }
  if (method === "DELETE" && effectiveId) {
    const doc = await d.collection("reports").doc(effectiveId).get();
    if (!doc.exists) return err("Not found", 404);
    if (doc.data()?.status === "LOCKED") return err("Locked", 400);
    await d.collection("reports").doc(effectiveId).delete();
    return ok({ message: "Deleted" });
  }
  if (method === "POST" && action === "submit" && effectiveId) {
    const doc = await d.collection("reports").doc(effectiveId).get();
    if (!doc.exists) return err("Not found", 404);
    if (doc.data()?.status !== "DRAFT") return err("Only DRAFT reports can be submitted", 400);
    await d.collection("reports").doc(effectiveId).update({ status: "SUBMITTED", submittedAt: new Date().toISOString() });
...
async function handleApprovals(req: NextRequest, id: string | undefined, action: string | undefined, method: string, user: any) {
  ...
  if (method === "POST" && !id) {
    ...
    return ok({ id: nid, reportId, level, status: "PENDING" }, 201);
  }
  if (method === "PUT" && id && action === "approve") {
    ...
    if (li + 1 < APPROVAL_LEVELS.length) {
      ...
      if (next === "LOCKED") {
        await d.collection("reports").doc(a.reportId).update({ status: "LOCKED", currentLevel: "LOCKED" });
      } else {
        await d.collection("approvals").doc(await genId()).set({ reportId: a.reportId, userId: user.id, level: next, status: "PENDING", comment: `Auto after ${a.level}`, createdAt: new Date().toISOString() });
        await d.collection("reports").doc(a.reportId).update({ currentLevel: next });
      }
    }
...
async function handleUsers(req: NextRequest, id: string | undefined, method: string, scope: string | null) {
  ...
  if (method === "GET" && !id) {
    ...
    if (scope) q = q.where("departmentId", "==", scope);
    const snap = await q.get();
    const items = snap.docs.map((dd: any) => { const { password, ...rest } = dd.data(); return { id: dd.id, ...rest }; });
    return ok(items);
  }
  if (method === "GET" && id) {
    const doc = await d.collection("users").doc(id).get();
    if (!doc.exists) return err("Not found", 404);
    const { password, ...rest } = doc.data()!;
    return ok({ id: doc.id, ...rest });
  }
  if (method === "POST" && !id) {
    ...
    return ok({ id: nid, email, name, role }, 201);
  }
  if (method === "PUT" && id) {
    const body = await req.json();
    ...
    await d.collection("users").doc(id).update(body);
    const doc = await d.collection("users").doc(id).get();
    const { password, ...rest } = doc.data()!;
    return ok({ id, ...rest });
  }
