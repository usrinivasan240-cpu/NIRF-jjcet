...
export default function PatentsPage() {
  ...
  const save = async () => {
    const id = editingId || "pat-" + Date.now().toString(36) + Math.random().toString(36).slice(2, 8);
    try {
      await setDoc(doc(db, "patents", id), { ...form, updatedAt: new Date().toISOString() }, { merge: true });
  ...
  const remove = async (id: string) => {
    ...
    try { await deleteDoc(doc(db, "patents", id)); load(); } catch (e) { console.error("Delete error:", e); }
  };
